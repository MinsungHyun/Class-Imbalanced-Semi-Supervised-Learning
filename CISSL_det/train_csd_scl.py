...
from csd_scl import build_ssd_con
import os
import sys
import time
import torch
...
parser.add_argument("--cb-cl", default=0.05, type=float,
                    help="hyperparameter of class-balanced consistency loss (default: 0).")
...
def train():
    ...
    while(finish_flag):
        ...
        if args.cb_cl>0:
            cls_imbal = torch.tensor(IMBALANCE).float().cuda()
            cbcl_weight = 20 * cls_imbal/cls_imbal.sum()
        ...
        for iteration in range(args.start_iter, cfg['max_iter']):
            ...
            if(mask_val.sum()>0):
                ...
                if args.cb_cl > 0:
                    mask_imbal_cls = conf_sampled[:, 1:].clone()

                    target_weights = torch.stack(list(map(lambda t: cbcl_weight[t.data], mask_imbal_cls.max(1)[1])))

                    consistency_conf_loss_a = conf_consistency_criterion(conf_sampled.log(),
                                                                         conf_sampled_flip.detach()).sum(-1)
                    consistency_conf_loss_b = conf_consistency_criterion(conf_sampled_flip.log(),
                                                                         conf_sampled.detach()).sum(-1)
                    consistency_conf_loss_a = torch.mean(torch.mul(consistency_conf_loss_a, target_weights))
                    consistency_conf_loss_b = torch.mean(torch.mul(consistency_conf_loss_b, target_weights))
            ...
            # consistency_loss = torch.div(consistency_conf_loss, 2) # + consistency_loc_loss
            # consistency_loss = torch.mul(torch.div(consistency_conf_loss, 2), 10) + consistency_loc_loss
...
def rampweight(iteration):
    ...
    return ramp_weight #torch.mul(ramp_weight, 10)
