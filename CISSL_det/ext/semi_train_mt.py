...
from ssd_consistency_mt import build_ssd_con
import os
import sys
import time
import torch
...
parser.add_argument('--dataset', default='VOC', choices=['VOC', 'COCO'], #/home/user/JISOO/detection/data/VOCdevkit/
...
def train():
    if args.dataset == 'COCO':
        ...
        cfg = voc
        # dataset = VOCCOCODetection_con(root=args.dataset_root,
        #                        transform=SSDAugmentation(cfg['min_dim'],
        #                                                  MEANS))
    ...
    while(finish_flag):
        ...
        loc_consistency_criterion = torch.nn.MSELoss(size_average=True).cuda()
        ...
        # print(args)
        log_txt = open("log/loc_no_ramp.txt",'w')
        ...
        if(args.start_iter==0):
            ...
            supervised_flag = 0
            supervised_dataset = VOCCOCODetection_con(root=args.dataset_root,
                                                     transform=SSDAugmentation(cfg['min_dim'],
        ...
        #unsupervised_dataset = VOCCOCODetection_con(root=args.dataset_root,
        #                           transform=SSDAugmentation(cfg['min_dim'],
        #                                                    MEANS))

       # unsupervised_data_loader = data.DataLoader(unsupervised_dataset, unsupervised_batch,
       #                               num_workers=args.num_workers,
       #                               shuffle=True, collate_fn=detection_collate,
       #                               pin_memory=True, drop_last=True)
        ...
        #unsupervised_batch_iterator = iter(unsupervised_data_loader)
        ...
        for iteration in range(args.start_iter, cfg['max_iter']):
            ...
            if(len(sup_image_index)!=0):
                try:
                    ...
                    # print('%3.2f, %3.2f, %d' % (loss_l.data.cpu().numpy()[0], loss_c.data.cpu().numpy()[0], len(targets)))
                except:
                    break
                    print('--------------')
            ...
            if(mask_val.sum()>0):
                ...
                # consistency_loc_loss = loc_consistency_criterion(loc_sampled, loc_sampled_flip)
                ...
                # consistency_loc_loss = torch.mean(torch.pow(loc_sampled - loc_sampled_flip, exponent=2))
                #
                # sampled_background = conf_sampled[:,0]
                # entropy_loss = torch.mean(torch.mul(torch.mul(conf_sampled.log(),conf_sampled),-0.1))
                # entropy_loss = torch.mean(torch.mul(conf_sampled.log() * conf_sampled).sum(1),-1))
            else:
                consistency_conf_loss = Variable(torch.cuda.FloatTensor([0]))
                consistency_loc_loss = Variable(torch.cuda.FloatTensor([0]))
                # entropy_loss = Variable(torch.cuda.FloatTensor([0]))


            # consistency_loss = torch.div(consistency_conf_loss,2) + consistency_loc_loss
            consistency_loss = consistency_loc_loss
            #+ consistency_loc_loss # + entropy_loss
            # consistency_loss = consistency_loc_loss
            ...
            if iteration % 10 == 0:
                ...
                print('lr : %.4f, super_len : %d\n' % (float(optimizer.param_groups[0]['lr']),len(sup_image_index)))

            #log_data = "loss: %.4f , loss_c: %.4f , loss_l: %.4f , loss_con: %.4f, lr : %.4f, super_len : %d\n" \
            #           % (loss.data, loss_c.data, loss_l.data, consistency_loss.data, float(optimizer.param_groups[0]['lr']),len(sup_image_index))
            #log_txt.write(log_data)
