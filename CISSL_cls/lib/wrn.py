...
class WRN_DML(nn.Module):
    """ WRN28-width with leaky relu (negative slope is 0.1)"""
    def __init__(self, width, num_classes, transform_fn=None):
        super().__init__()

        self.init_conv_1 = conv3x3(3, 16)
        self.init_conv_2 = conv3x3(3, 16)
        ...
        unit3_1 = [residual(filters[2], filters[3], 2)] + \
            [residual(filters[3], filters[3]) for _ in range(1, 4)]
        unit3_2 = [residual(filters[2], filters[3], 2)] + \
                  [residual(filters[3], filters[3]) for _ in range(1, 4)]

        self.unit3_1 = nn.Sequential(*unit3_1)
        self.unit3_2 = nn.Sequential(*unit3_2)

        self.unit4_1 = nn.Sequential(*[BatchNorm2d(filters[3]), relu(), nn.AdaptiveAvgPool2d(1)])
        self.unit4_2 = nn.Sequential(*[BatchNorm2d(filters[3]), relu(), nn.AdaptiveAvgPool2d(1)])

        self.output_1 = nn.Linear(filters[3], num_classes)
        self.output_2 = nn.Linear(filters[3], num_classes)
    ...
    def forward(self, x, return_feature=False):
        if self.training and self.transform_fn is not None:
            x_1 = self.transform_fn(x)
            x_2 = self.transform_fn(x)
        else:
            x_1 = x.clone()
            x_2 = x.clone()
        x_1 = self.init_conv_1(x_1)
        x_1 = self.unit1(x_1)
        x_1 = self.unit2(x_1)
        x_1 = self.unit3_1(x_1)

        x_2 = self.init_conv_2(x_2)
        x_2 = self.unit1(x_2)
        x_2 = self.unit2(x_2)
        x_2 = self.unit3_2(x_2)

        f_1 = self.unit4_1(x_1)
        f_2 = self.unit4_2(x_2)

        c_1 = self.output_1(f_1.squeeze())
        c_2 = self.output_2(f_2.squeeze())
        if return_feature:
            return c_1, c_2, f_1, f_2
        else:
            return c_1, c_2
